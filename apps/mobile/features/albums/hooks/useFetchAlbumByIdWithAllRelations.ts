 ...
 *
 * if (isLoading) return <Text>Loading detailed album...</Text>;
 * if (isError) return <Text>Error loading detailed album!</Text>;
 *
 * return <Text>{album?.name} by {album?.artists?.[0]?.name} with {album?.songs?.length} songs</Text>;
 * ```
 */
