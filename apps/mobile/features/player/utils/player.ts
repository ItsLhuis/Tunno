...
/**
 * Resolves a `SongWithMainRelations` object into a `Track` object compatible with `react-native-track-player`.
...
export async function resolveTrack(song: SongWithMainRelations): Promise<Track> {
  ...
  if (song.thumbnail) {
    ...
    artworkUrl = defaultArtwork // Use a default artwork if no thumbnail is available.
  }

  return {
    ...song,
    id: song.id, // Ensure `id` is properly mapped to TrackPlayer's `id`
    url: audioUrl,
    title: song.name,
    ...
    isLiveStream: false // Assuming songs are not live streams
  }
}
...
export async function getSongFromCacheOrFetch(id: number): Promise<SongWithMainRelations | null> {
  ...
  if (song) {
    // If fetched, set the data in the query client cache for future use.
...
export async function prefetchSongs(ids: number[]): Promise<void> {
  // Filter out IDs that are already present in the cache to avoid unnecessary fetches.
  ...
  // Store the fetched songs in the query client cache.
  songs.forEach((song) => {
